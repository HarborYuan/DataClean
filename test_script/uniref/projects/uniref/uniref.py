# Modified by Haobo
# changed dump mask to directly return
...
class UniRef(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.detr = DDETRsegm(cfg, model, freeze_detr=self.freeze_detr, ota=self.ota, rel_coord=self.use_rel_coord,
                              new_mask_head=self.new_mask_head, use_raft=self.use_raft,
                              mask_out_stride=self.mask_stride, mask_out_stride_vos=self.mask_stride_vos,
                              with_lang_ref=self.with_lang_ref, with_mask_ref=self.with_mask_ref,
                              use_early_fusion=self.use_early_fusion, num_frames=self.num_frames,
                              backbone_channels=self.backbone_channels, use_backbone_feature=self.use_backbone_feature,
        ...
        # Criterion
        self.criterion = SetCriterion(self.num_classes, matcher, weight_dict, losses, focal_alpha=focal_alpha,
                                      ota=self.ota,
        ...
        self.save_refdavis_prefix = os.path.join(cfg.OUTPUT_DIR, "inference",
                                                 "refdavis")  # for saving ref-davis results
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            if task == "detection":
                ...
                ref_masks = [target["masks"] for target in
                             ref_targets]  # list[tensor], length of bs, [1, H, W], image size
                ...
                # rvos data
                    ...
                    if 0. < torch.rand(1) < 0.4:  # only fuse with language
                        ...
                        ref_masks = [target["masks"] for target in
                                     ref_targets]  # list[tensor], length of bs, [1, H, W], image size
                        # forward to get the mask_dict_features
                        srcs_ref, masks_ref, _, _ = self.forward_ref_backbone(
                            ref_images)  # multi-scale features, 8x -> 64x
                        mask_dict_features = self.forward_template(ref_images, ref_masks, srcs_ref, masks_ref)
                        output, loss_dict = self.detr.coco_forward(det_images, det_targets, self.criterion, train=True,
                                                                   lang_dict_features=lang_dict_features,
                                                                   mask_dict_features=mask_dict_features, task=task)
            ...
            # video task
                ...
                if task == "vos":
                    ...
                    if dataset_name.startswith("refytvos"):
                        return self.inference_rvos_vl(batched_inputs, images, aggregate_objects=False)
    ...
    def prepare_targets(self, targets):
        ...
        for targets_per_image in targets:
            ...
            if hasattr(targets_per_image, "gt_masks"):
                ...
                gt_masks_pad = torch.zeros((n_inst, max_size[0], max_size[1]), device=gt_masks.device,
                                           dtype=gt_masks.dtype)
                gt_masks_pad[:, :hm, :wm].copy_(gt_masks)
                new_targets.append(
                    {"labels": gt_classes, "boxes": gt_boxes, 'masks': gt_masks_pad, "image_size": image_size_xyxy,
    ...
    def prepare_image_targets_boxinst(self, batched_inputs, size_divisibility=32):
        ...
        original_image_masks = [torch.ones_like(x[0], dtype=torch.float32) for x in
                                original_images]  # [torch.ones(H, W),...] len=bs
    ...
    def prepare_targets_boxinst(self, targets):
        ...
        for targets_per_image in targets:
            ...
            new_targets.append(
                {"labels": gt_classes, "boxes": gt_boxes, 'masks': gt_masks, "image_size": image_size_xyxy,
    ...
    def inference_vos_3f(self, batched_inputs, task="vos"):
        ...
        for frame_idx in range(video_len):
            ...
            for obj_id in ref_dict_init.keys():
                ...
                for _, (logits, output_masks, output_boxes, image_size) in enumerate(zip(
                    ...
                    if topk_values > score_thres:
                        ...
                        track_masks = F.interpolate(track_masks, size=(
                        output_h * self.mask_stride_vos, output_w * self.mask_stride_vos), mode="bilinear",
                                                    align_corners=False).sigmoid()
                        track_masks = track_masks[:, :, :image_size[0], :image_size[1]]  # crop the padding area
                        track_masks = F.interpolate(track_masks, size=(height, width), mode='bilinear',
                                                    align_corners=False)  # (1, 1, H, W), resize to original size
            ...
            if len(cur_gt_instances) > 0:
                ...
                cur_obj_ids = cur_gt_instances.ori_id
                num_new_obj = len(cur_obj_ids)
                gt_masks = gt_targets[frame_idx]["masks"][:, :image_size[0],
                           :image_size[1]]  # here image size is from the images.image_sizes above
                gt_masks = \
                F.interpolate(gt_masks[None].float(), size=(height, width), mode="bilinear", align_corners=False)[0]
                for obj_idx in range(num_new_obj):
                    cur_obj_id = cur_obj_ids[obj_idx]
            ...
            for cur_id in cur_obj_ids:
                ...
                try:
                    ...
                    cur_mask_tensor_rsz = \
                    F.interpolate(cur_mask_tensor[None].float(), size=(image_size[0], image_size[1]))[0]
    ...
    def inference_rvos_vl(self, batched_inputs, images, aggregate_objects=False, task="rvos"):
                ...
                if not ref_init_dict:  # the first frame, fuse L to generate mask
                    ...
                    for _, (logits, output_mask, output_boxes, image_size) in enumerate(zip(
                        ...
                        if topk_values > score_thres:
                            ...
                            track_masks = F.interpolate(track_masks,
                                                        size=(output_h * self.mask_stride, output_w * self.mask_stride),
                                                        mode="bilinear", align_corners=False).sigmoid().to()
                            track_masks = track_masks[:, :, :image_size[0], :image_size[1]]  # crop the padding area
                            track_masks = F.interpolate(track_masks, size=(height, width), mode='bilinear',
                                                        align_corners=False)  # (1, 1, H, W), resize to original size
        ...
        # for ref-davis, all objects should be aggregated in one video
                ...
                for frame_idx in range(video_length):
                    ...
                    # step 2: following frames, start tracking with language and predicted mask
                        ...
                        if (cur_mask > 0).any():
                            ...
                            cur_ref_masks = [cur_mask_tensor_final]
                            srcs_ref, masks_ref, _, _ = self.forward_ref_backbone(clip_images)
                            cur_mask_dict_features = self.forward_template(clip_images, cur_ref_masks, srcs_ref,
                                                                           masks_ref)
                            ref_prev_dict = cur_mask_dict_features
            ...
            for frame_idx in range(video_length):
                ...
                mask_merge = torch.zeros((height, width, len(cur_obj_ids_int) + 1),
                                         device=self.device)  # [H, W, N_obj+1]
                tmp_list = []
    ...
    def inference(self, box_cls, box_pred, mask_pred, image_sizes, binary_mask=True, task="detection"):
        ...
        for i, (logits_per_image, box_pred_per_image, image_size) in enumerate(zip(
            ...
            if self.mask_on:
                ...
                mask = F.interpolate(mask_pred_i, size=(H * self.mask_stride, W * self.mask_stride), mode='bilinear',
                                     align_corners=False)
                if binary_mask:
    ...
    def forward_text(self, captions, device):
        if isinstance(captions[0], str):
            tokenized = self.tokenizer.batch_encode_plus(
                captions, padding="max_length", truncation=True, max_length=self.context_len, return_tensors="pt").to(
                device)
            ...
            if self.lang_pool:
                ...
                text_sentence_features = embedded.sum(1) / (
                    text_word_masks.ne(1).sum(-1).unsqueeze(-1).float())  # [batch_size, C]
            else:
    ...
    def forward_ref_backbone(self, samples):
        ...
        for l, (feat, pos_l) in enumerate(
                zip(features[-self.num_transformer_features:], pos[-self.num_transformer_features:])):
            # src: [N, _C, Hi, Wi],
            # mask: [N, Hi, Wi],
            # pos: [N, C, H_p, W_p]
...
def concat_mask_dict_features(ref_feats_dict1, ref_feats_dict2):
    ...
    for n_l in range(level):
        ...
        ref_value = torch.cat([ref_feats_dict1["ref_values"][n_l], ref_feats_dict2["ref_values"][n_l]],
                              dim=1)  # [B, HiWi+HiWi, C]
        mask = torch.cat([ref_feats_dict1["masks"][n_l], ref_feats_dict2["masks"][n_l]], dim=1)  # [B, HiWi+HiWi]
        embedded = torch.cat(
            [ref_feats_dict1["ref_embeds"][n_l].unsqueeze(1), ref_feats_dict2["ref_embeds"][n_l].unsqueeze(1)], dim=1)
