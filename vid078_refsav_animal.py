...
def find_first_noun(sentence):
    ...
    for token in doc:
        # 检查词性是否为名词
        if token.pos_ == "NOUN" and token.text != 'object':
            return token.text

    print("Warning: no NOUN : {}".format(sentence))
    return "object"

PATH = 'data/video_datas/ref_SAV/meta_expressions_valid.json'
MASK_PATH = 'data/video_datas/ref_SAV/mask_dict.json'
OUT_DIR = 'data/vid/078_refsav_animal'

IMG_PREFIX = 'data/video_datas/ref_SAV/videos'

SELECT = []
START = []
FIRST = ['cat', 'dog', 'bird', 'horse', 'animal', 'bear', 'goose', 'sheep', 'koala', 'elephant', 'flamingo', 'lizard', 'varanus', 'lamb']
